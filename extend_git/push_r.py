...
def git_push(folder_path):
    try:
        subprocess.check_output(['git', 'push'], cwd=folder_path, stderr=subprocess.STDOUT, universal_newlines=True)
        print(f"Git push successful in {folder_path}")
    except subprocess.CalledProcessError as e:
        print(f"Error in git push for {folder_path} : {e.output.splitlines()[0]}")

def process_folder(folder_path, queue):
    if os.path.exists(os.path.join(folder_path, '.git')):
        git_push(folder_path)
    else:
        try:
